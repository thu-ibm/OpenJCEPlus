...
Java_com_ibm_crypto_plus_provider_openssl_OpenSSLNativeInterface_PBKDF2_1derive(
    ...
    if (debug) {
        gslogMessage("DETAIL_PBKDF2 Algorithm: %s, Iterations: %d, KeyLength: %d",
                    algoName, iterations, keyLength);
    }
#endif

    // Get password bytes
    jbyte* passwordBytes = getByteArrayElementsSafe(env, password, functionName, "Password is NULL");
    ...
    // Get salt bytes
    jbyte* saltBytes = getByteArrayElementsSafe(env, salt, functionName, "Salt is NULL");
    if (saltBytes == NULL) {
    ...
    unsigned char* derivedKey = (unsigned char*)mallocSafe(env, keyLength,
                                                            "Failed to allocate memory for derived key");
    if (derivedKey == NULL) {
    ...
    // Fetch KDF
    if (debug) {
        gslogMessage("DETAIL_PBKDF2 About to fetch PBKDF2 KDF, context=%p, libctx=%p",
                    context, context ? context->libctx : NULL);
    }
    
    EVP_KDF* kdf = fetchKDFSafe(env, context, "PBKDF2", functionName,
    ...
    if (kctx == NULL) {
        ...
        cleanupByteArray(env, password, passwordBytes, JNI_ABORT);
        cleanupStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_PBKDF2_FAILED, "Failed to create PBKDF2 context");
    ...
    params[0] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD,
                    passwordBytes, (size_t)passwordLen);
    ...
    // Perform derivation
    if (debug) {
        gslogMessage("DETAIL_PBKDF2 About to call EVP_KDF_derive: kctx=%p, derivedKey=%p, keyLength=%d",
                    kctx, derivedKey, keyLength);
        gslogMessage("DETAIL_PBKDF2 Params: password=%p (len=%d), salt=%p (len=%d), iter=%u, digest=%s",
                    passwordBytes, passwordLen, saltBytes, saltLen, (unsigned int)iterations, algoName);
    }
    ...
    if (result != 1) {
        ...
        cleanupStringUTFChars(env, digestAlgo, algoName);
        free(derivedKey);
        throwOpenSSLException(env, OPENSSL_PBKDF2_FAILED, "PBKDF2 derivation failed");
    ...
    jbyteArray result_array = newByteArraySafe(env, keyLength, "Failed to create result array");
    ...
    (*env)->SetByteArrayRegion(env, result_array, 0, keyLength, (jbyte*)derivedKey);
    free(derivedKey);
