...
Java_com_ibm_crypto_plus_provider_openssl_OpenSSLNativeInterface_HKDF_1extract(
    ...
    EVP_MD* md = (EVP_MD*)fetchDigestSafe(env, context, algoName, functionName,
                                          OPENSSL_HKDF_EXTRACT_FAILED, "Failed to fetch digest algorithm");
    if (md == NULL) {
    ...
    unsigned char* prk = (unsigned char*)mallocSafe(env, digestSize,
                                                     "Failed to allocate memory for PRK");
    if (prk == NULL) {
        EVP_MD_free(md);
        cleanupByteArray(env, ikm, ikmBytes, JNI_ABORT);
        if (saltBytes != NULL) {
    ...
    // Create KDF context
    EVP_KDF* kdf = fetchKDFSafe(env, context, "HKDF", functionName,
    ...
    if (kctx == NULL) {
        ...
        cleanupStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_EXTRACT_FAILED, "Failed to create HKDF context");
    ...
    params[1] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char*)algoName, 0);
    params[2] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, ikmBytes, ikmLen);
    
    if (saltBytes != NULL && saltLen > 0) {
        params[3] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, saltBytes, saltLen);
    ...
    if (result != 1) {
        ...
        cleanupStringUTFChars(env, digestAlgo, algoName);
        free(prk);
        throwOpenSSLException(env, OPENSSL_HKDF_EXTRACT_FAILED, "HKDF extract failed");
    ...
    jbyteArray result_array = newByteArraySafe(env, prkLen, "Failed to create result array");
...
Java_com_ibm_crypto_plus_provider_openssl_OpenSSLNativeInterface_HKDF_1expand(
...
#ifdef DEBUG_HKDF_DETAIL
    if (debug) {
        gslogMessage("DETAIL_HKDF Expand - Algorithm: %s, Length: %d", algoName, length);
    }
#endif
    ...
    if (prkBytes == NULL) {
        ...
        throwOpenSSLException(env, OPENSSL_INVALID_PARAMETER, "PRK is NULL");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    unsigned char* okm = (unsigned char*)mallocSafe(env, length,
    ...
    if (md == NULL) {
        ...
        (*env)->ReleaseByteArrayElements(env, prk, prkBytes, JNI_ABORT);
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_EXPAND_FAILED, "Failed to fetch digest algorithm");
    ...
    if (kdf == NULL) {
        ...
        (*env)->ReleaseByteArrayElements(env, prk, prkBytes, JNI_ABORT);
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_EXPAND_FAILED, "Failed to fetch HKDF");
    ...
    if (kctx == NULL) {
        ...
        (*env)->ReleaseByteArrayElements(env, prk, prkBytes, JNI_ABORT);
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_EXPAND_FAILED, "Failed to create HKDF context");
        logOpenSSLError("EVP_KDF_CTX_new");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    params[0] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_MODE, &mode);
    params[1] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char*)algoName, 0);
    params[2] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, prkBytes, prkLen);
    
    if (infoBytes != NULL && infoLen > 0) {
        params[3] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, infoBytes, infoLen);
        params[4] = OSSL_PARAM_construct_end();
    } else {
        params[3] = OSSL_PARAM_construct_end();
    }
    ...
    if (result != 1) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        free(okm);
        throwOpenSSLException(env, OPENSSL_HKDF_EXPAND_FAILED, "HKDF expand failed");
        logOpenSSLError("EVP_KDF_derive");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    // Create Java byte array for result
    jbyteArray result_array = newByteArraySafe(env, length, "Failed to create result array");
    if (result_array == NULL) {
        free(okm);
        throwOpenSSLException(env, OPENSSL_ALLOCATION_FAILED, "Failed to create result array");
        logFunctionExit(functionName);
        return NULL;
    }
...
Java_com_ibm_crypto_plus_provider_openssl_OpenSSLNativeInterface_HKDF_1derive(
...
#ifdef DEBUG_HKDF_DETAIL
    if (debug) {
        gslogMessage("DETAIL_HKDF Derive - Algorithm: %s, Length: %d", algoName, length);
    }
#endif
    ...
    if (ikmBytes == NULL) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_INVALID_PARAMETER, "IKM is NULL");
        logFunctionExit(functionName);
        return NULL;
    }
    jsize ikmLen = (*env)->GetArrayLength(env, ikm);
    ...
    if (okm == NULL) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_ALLOCATION_FAILED, "Failed to allocate memory for OKM");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    if (md == NULL) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_DERIVE_FAILED, "Failed to fetch digest algorithm");
        logOpenSSLError("EVP_MD_fetch");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    if (kdf == NULL) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_DERIVE_FAILED, "Failed to fetch HKDF");
        logOpenSSLError("EVP_KDF_fetch");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    if (kctx == NULL) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        throwOpenSSLException(env, OPENSSL_HKDF_DERIVE_FAILED, "Failed to create HKDF context");
        logOpenSSLError("EVP_KDF_CTX_new");
        logFunctionExit(functionName);
        return NULL;
    }
    ...
    params[paramIdx++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char*)algoName, 0);
    params[paramIdx++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, ikmBytes, ikmLen);
    
    if (saltBytes != NULL && saltLen > 0) {
        params[paramIdx++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, saltBytes, saltLen);
    }
    
    if (infoBytes != NULL && infoLen > 0) {
        params[paramIdx++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, infoBytes, infoLen);
    }
    ...
    if (result != 1) {
        ...
        (*env)->ReleaseStringUTFChars(env, digestAlgo, algoName);
        free(okm);
        throwOpenSSLException(env, OPENSSL_HKDF_DERIVE_FAILED, "HKDF derive failed");
        logOpenSSLError("EVP_KDF_derive");
        logFunctionExit(functionName);
        return NULL;
    }
