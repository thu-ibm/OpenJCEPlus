...
/* HMAC context struct (formerly in OpenSSLHMAC.h) */
typedef struct {
    HMAC_CTX*     hmacCtx;
    const EVP_MD* md;
    int           macSize;
...
Java_com_ibm_crypto_plus_provider_openssl_NativeOpenSSLImplementation_HMAC_1doFinal(
    ...
    if (macOffset < 0) {
        setPendingOpenSSLException(env, OPENSSL_HMAC_INVALID,
                              "macOffset must not be negative");
        logFunctionExit(functionName);
        return -1;
    }

    jsize outputLen = (*env)->GetArrayLength(env, output);
    if (outputLen - macOffset < hmacCtx->macSize) {
        setPendingOpenSSLException(env, OPENSSL_HMAC_INVALID,
                              "Output array too small for HMAC at given offset");
        logFunctionExit(functionName);
        return -1;
    }
    ...
    // Finalize HMAC — write directly at macOffset within the pinned buffer
    unsigned int macLen = 0;
    int          result =
        HMAC_Final(hmacCtx->hmacCtx,
                   (unsigned char*)(outputBuffer + macOffset), &macLen);

    if (result != 1) {
        cleanupByteArray(env, output, outputBuffer, JNI_ABORT);
...
#ifdef DEBUG_HMAC_DETAIL
    if (debug) {
        // Debug log must occur before ReleaseByteArrayElements to avoid use-after-free
        gslogMessage("DETAIL_HMAC Finalized HMAC, size: %d bytes", macLen);
        gslogMessageHex((char*)(outputBuffer + macOffset), 0, macLen, 0, 0, NULL);
    }
#endif
